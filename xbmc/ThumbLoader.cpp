...
bool CThumbLoader::LoadRemoteThumb(CFileItem *pItem)
{
  // look for remote thumbs
    CLog::Log(LOGDEBUG,"Efren Thumbnail image loading %s\n",pItem->GetThumbnailImage().c_str());
  CStdString thumb(pItem->GetThumbnailImage());
  if (!g_TextureManager.CanLoad(thumb))
  {
    CLog::Log(LOGDEBUG,"Efren Cant Load %s\n",pItem->GetThumbnailImage().c_str());
    CStdString cachedThumb(pItem->GetCachedVideoThumb());
    if (CFile::Exists(cachedThumb)) {
      pItem->SetThumbnailImage(cachedThumb);
	    CLog::Log(LOGDEBUG,"Efren Thumbnail Cached Thumb %s    %s\n",cachedThumb.c_str(),(pItem->HasThumbnail())?"true":"false");	
}
    else
    {
...
bool CThumbLoader::CheckAndCacheThumb(CFileItem &item)
{
  if (item.HasThumbnail() && !g_TextureManager.CanLoad(item.GetThumbnailImage()))
  {
    ...
    CLog::Log(LOGDEBUG,"Efren Cache Remote Thumbnail image loading %s\n",thumb.c_str());
...
bool CVideoThumbLoader::LoadItem(CFileItem* pItem)
{
  ...
  if (!pItem->HasThumbnail())
  {
    ...
    CLog::Log(LOGDEBUG,"Efren Thumbnail Not special image loading %s\n",pItem->GetThumbnailImage().c_str());
