...
bool CMediaManager::LoadSources()
{
  ...
  m_locations.clear();
CLog::Log(LOGDEBUG, "Load sources LOLAILOLA\n");
  // load xml file...
  ...
  if (pNetwork)
  {
	...
	CLog::Log(LOGDEBUG, "Load sources LOLAILOLA ITER\n");
...
void CMediaManager::GetNetworkLocations(VECSOURCES &locations)
{
  // Load our xml file
  LoadSources();
   CLog::Log(LOGDEBUG, "Network Locations size LOLAILOLA = %d\n", m_locations.size());
  for (unsigned int i = 0; i < m_locations.size(); i++)
  {
	...
	CLog::Log(LOGDEBUG, "Network locations LOLAILOLA ITER\n");
  }
}
...
bool CMediaManager::AddNetworkLocation(const CStdString &path)
{
  ...
  m_locations.push_back(location);
CLog::Log(LOGDEBUG, "Network Locations PUSHED LOLAILOLA\n");
  return SaveSources();
}

bool CMediaManager::HasLocation(const CStdString& path) const
{
CLog::Log(LOGDEBUG, "Has Location LOLAILOLA\n");
  for (unsigned int i=0;i<m_locations.size();++i)
  {
...
{
CLog::Log(LOGDEBUG, "Remove LOCATION LOLAILOLA\n");
  for (unsigned int i=0;i<m_locations.size();++i)
  {
    if (m_locations[i].path == path)
    {
...
{
CLog::Log(LOGDEBUG, "Set Location LOLAILOLA\n");
  for (unsigned int i=0;i<m_locations.size();++i)
  {
