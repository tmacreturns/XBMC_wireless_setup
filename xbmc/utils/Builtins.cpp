...
const BUILT_IN commands[] = {
  ...
  { "SetThumbnails",              false,  "Set Thumbnails folder"},
...
int CBuiltins::Execute(const CStdString& execString)
{
  ...
  {
   /*CLog::Log(LOGDEBUG, "Efren SendClick recognized\n");
    if (params.size() == 2)
    {
      // have a window - convert it
      int windowID = CButtonTranslator::TranslateWindow(params[0]);
	   CLog::Log(LOGDEBUG, "Efren SendClick recognized number %d\n",windowID);
      CGUIMessage message(GUI_MSG_CLICKED, atoi(params[1].c_str()), windowID);
      g_windowManager.SendMessage(message);
    }
    else
    { // single param - assume you meant the active window
      if  (atoi(params[0].c_str()) == 8)
	{
		//bool test=false;
		CStdString strPath("musicsearch://A/");
	 CLog::Log(LOGDEBUG, "Efren SendClick recognized musicsearch %s\n",strPath.c_str());
		g_windowManager.ActivateWindow(WINDOW_MUSIC_NAV);
		//while (!test) test = CGUIDialogYesNo::ShowAndGetInput(39999,20022,20022,20022);
	}
      else {
    ...
    { // single param - assume you meant the active window
      if (atoi(params[0].c_str()) == 222)    
	{  	
		int userDecision = 2;
		CLog::Log(LOGDEBUG, "Efren SendClick recognized Params NOT 2      %d\n",atoi(params[0].c_str()));
		CGUIDialogSelect *pDlg = (CGUIDialogSelect*)g_windowManager.GetWindow(WINDOW_DIALOG_SELECT);
		if (pDlg)
  		{
		    pDlg->SetHeading("Select which Library to Search\n");
		    pDlg->Reset();
		    pDlg->Add("Music");
   		    pDlg->Add("Video");
		...
		userDecision = pDlg->GetSelectedLabel();
		if (userDecision == 0)  
		{
			g_windowManager.ActivateWindow(10502);
			CGUIMessage message(GUI_MSG_CLICKED, 8, g_windowManager.GetActiveWindow()); 
			g_windowManager.SendMessage(message);
		}
		else if (userDecision == 1)
		{
			g_windowManager.ActivateWindow(10025);
			CGUIMessage message(GUI_MSG_CLICKED, 8, g_windowManager.GetActiveWindow()); 
			g_windowManager.SendMessage(message);
		}
		//else do nothing
  ...
  else if (execute.Equals("setthumbnails"))
  {
	 CStdString path;

	 VECSOURCES *shares = NULL;
         shares = g_settings.GetSourcesFromType("folder");
         ...
         if (!shares)
         {
	        ...
	        g_mediaManager.GetLocalDrives(localShares);
		CLog::Log(LOGDEBUG, "Local Drives  %d %s\n",localShares.size(),localShares.front().strName.c_str());
	        g_mediaManager.GetNetworkLocations(networkShares);
		CLog::Log(LOGDEBUG, "Network Drives  %d \n",networkShares.size());
...
/******************/
	...
	localShares.push_back(share1);

          /*VECSOURCES networkShares;
          g_mediaManager.GetLocalDrives(shares);
          g_mediaManager.GetNetworkLocations(networkShares);

          shares.insert(shares.end(), networkShares.begin(), networkShares.end());*/
         
	  CProfile *profile = g_settings.GetProfile(0);
	  CGUIDialogFileBrowser::ShowAndGetDirectory(localShares,g_localizeStrings.Get(110), path, true);
	  //CLog::Log(LOGDEBUG, "BUILT IN %s    \n",path.c_str());
	  profile->setThumb(path);
	  //CLog::Log(LOGDEBUG, "BUILT IN  (2) %s   %d \n",g_settings.GetMasterProfile().getThumb().c_str(),localShares.size());
