...
//if (key.FromKeyboard()) CLog::Log(LOGDEBUG, "Efren On Keyboard Event  %u\n",key.GetButtonCode());

  // this will be checked for certain keycodes that need
  // special handling if the screensaver is active
  CAction action = CButtonTranslator::GetInstance().GetAction(iWin, key);
//CLog::Log(LOGDEBUG, "Efren On Keyboard Event  ACTION %u\n",action.GetID());
  // a key has been pressed.
  // reset Idle Timer
  m_idleTimer.StartZero();
  bool processKey = AlwaysProcess(action);

//if (processKey) CLog::Log(LOGDEBUG, "Efren Process Key Always true \n");
  ResetScreenSaver();
  ...
  { // fullscreen info dialog - special case
    ...
    //CLog::Log(LOGDEBUG, "Efren Window Dialog Fullscreen INfo \n");
    g_Keyboard.Reset();
  ...
  else
  {
    ...
    if (useKeyboard)
    {
	//CLog::Log(LOGDEBUG, "Efren On Keyboard Event  ACTION (2) %u\n",action.GetID());
      action = CAction(0); // reset our action
//CLog::Log(LOGDEBUG, "Efren On Keyboard Event  ACTION (3) %u\n",action.GetID());
...
//CLog::Log(LOGDEBUG, "Efren On Keyboard Event  ACTION (4) %u %u\n",action.GetID(),key.GetButtonCode());
      ...
      if (!action.GetID())
      {
		...
		//CLog::Log(LOGDEBUG, "Efren Get Unicode True %u    <-> %u\n", g_Keyboard.GetAscii(), g_Keyboard.GetUnicode());
		}
          else
		{
		/*uint32_t maskedKey = key.GetButtonCode() & 0xFFFFFF00;
		if (maskedKey == 0x0000F000) {
			CLog::Log(LOGDEBUG, "Efren Remote key pressed %u\n", (key.GetButtonCode() & 0x000000FF ));
			action = CAction((key.GetButtonCode() & 0x000000FF ) | KEY_ASCII);
		}
		else
		{
			CLog::Log(LOGDEBUG, "Efren Get Unicode False %u    <-> %u   ----->   %u\n", g_Keyboard.GetAscii(), g_Keyboard.GetVKey(),key.GetButtonCode());
